...
def fetch_votaciones_2024():
    # URL to get the list of votaciones for 2024
    url_votaciones_2024 = 'https://opendata.camara.cl/camaradiputados/WServices/WSLegislativo.asmx/retornarVotacionesXAnno?prmAnno=2024'

    try:
        # Fetch the XML data
        response = requests.get(url_votaciones_2024)
        ...
        # Save the JSON data
        with open('data/votacionesAño_2024.json', 'w', encoding='utf-8') as f:
            f.write(json_data)

        print("votacionesAño_2024.json successfully updated.")
...
def fetch_votaciones_details():
    ...
    data_dict = fetch_votaciones_2024()
    if data_dict is None:
        print("Failed to fetch votaciones for 2024. Exiting.")
        return
    ...
    for votacion in votaciones_list:
        ...
        details_filename = f'data/votaciones_detalles/detallesVotacion_{votacion_id}.json'
