...
def fetch_votaciones_2002():
    # URL to get the list of votaciones for 2024
    url_votaciones_2002 = 'https://opendata.camara.cl/camaradiputados/WServices/WSLegislativo.asmx/retornarVotacionesXAnno?prmAnno=2002'

    try:
        # Fetch the XML data
        response = requests.get(url_votaciones_2002)
        ...
        # Save the JSON data
        with open('data/votacionesAño_2002.json', 'w', encoding='utf-8') as f:
            f.write(json_data)

        print("votacionesAño_2002.json successfully updated.")
...
def fetch_votaciones_details():
    ...
    data_dict = fetch_votaciones_2002()
    if data_dict is None:
        print("Failed to fetch votaciones for 2002. Exiting.")
        return
    ...
    for votacion in votaciones_list:
        ...
        details_filename = f'data/votaciones_detalles/2002/detallesVotacion_{votacion_id}.json'
